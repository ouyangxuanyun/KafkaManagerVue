    ...
    for (var brokerid = 0; brokerid < brokernum; brokerid++) {
      !function (brokerid) {
        getbrokerinfo(children[brokerid], function (data) {//console.log(data);
          ...
          BrokerList[brokerid][0] = children[brokerid];
          ...
          for (var i = 0; i < children.length; i++) {
            !function (i) {
              getPartiOwner(consumergp, topic, children[i], function (data) {
                ...
                if (i == children.length - 1) callback(null, Parti_Owner)
              })
            }(i)
          }
        }
  ...
  getAllConsumeInfo(function (allconsumeInfo) {
    for (var i = 0; i < allconsumeInfo.length; i++) {
      !function (i) {
        ...
        if (i == allconsumeInfo.length - 1) callback(null, result);
      }(i)
    }
  })
}
