...
function connecthost(host, port, callback) {
  ...
  client.on("connect", function () {
    ...
    for (var i = 0; i < 6; i++) {
      !function (i) {
        getmetricrow(client, "kafka.server:type=BrokerTopicMetrics,name=" + Items[i], function (err, data) {
          if (err) {
  ...
  getMBeanList(cluster_arr, brokers, function (err, mb_data) {
    ...
    async.each(topic_arr, function (topic, callback) {
      ...
      async.each(mb_data, function (mb_item, callback) {
        ...
        if (err) return callback(new Error(err.stack))
...
function getTopicJMXdata(cluster_arr, brokers, topic, callback) {
  ...
  getMBeanList(cluster_arr, brokers, function (err, mb_data) {
    ...
    async.each(mb_data, function (mb_item, callback) {
          ...
          if (underReplicated_arr.length != 0) {
            async.each(underReplicated_arr, function (partition_item, callback) {
              if (mb_item.partition == partition_item.partition) {
                ...
                callback(null, 'the partition is existed!');
              }
              else callback();
            }, function (err) {
              if (err) {
      ...
      for (var i = 0; i < 4; i++) {
        !function (i) {
          if (BytesInPerSec[i] < 100) {
            BytesInPerSec_str[i] = BytesInPerSec[i].toFixed(2);
          }
          else BytesInPerSec_str[i] = (BytesInPerSec[i] / 1000).toFixed(1) + 'k';
          if (BytesOutPerSec[i] < 100) {
            BytesOutPerSec_str[i] = BytesOutPerSec[i].toFixed(2);
          }
          else BytesOutPerSec_str[i] = (BytesOutPerSec[i] / 1000).toFixed(1) + 'k';
          if (BytesRejectedPerSec[i] < 100) {
            BytesRejectedPerSec_str[i] = BytesRejectedPerSec[i].toFixed(2);
          }
          else BytesRejectedPerSec_str[i] = (BytesRejectedPerSec[i] / 1000).toFixed(1) + 'k';
        }(i);
      }
      metrics.BytesInPerSec_str = BytesInPerSec_str
      metrics.BytesOutPerSec_str = BytesOutPerSec_str
      metrics.BytesRejectedPerSec_str = BytesRejectedPerSec_str
      //end add 12-08
      ...
      callback(null, {
        topic: topic,
        underReplicated_arr: underReplicated_arr,
        under_replicated: u_r_pnum,
        logEndPartition_arr: logEndPartition_arr,
        end_offset: end_offset,
        logStartPartition_arr: logStartPartition_arr,
        start_offset: start_offset,
        offset: (end_offset - start_offset),
        metrics: metrics
      })
